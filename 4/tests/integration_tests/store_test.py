...
class TestStoreWithMock(unittest.TestCase):
    ...
    def test_connection_counts(self, _, key, value):        
        store = Store()
        store.store.append.side_effect = [Exception() for i in range(6)]        
        store.set(key, value) 
        self.assertEqual(store.store.append.call_count, 6)
    ...
    def test_store_methods_cache_set_cache_get_with_expir(self, _, key, value, expir):
        ...
        store.store.append.assert_called_with('cash_' + key, value) 
...
### ----------------  set|get methods -----------------------
    ...
    def test_set_int_float_without_expiration(self, key, value):
    ...
    def test_set_list_without_expiration(self, key, value):
    ...
    def test_set_expired(self, key, value):
        if isinstance(value, list):
            value = json.dumps(value)
            
        self.store.set(key, value, 1)
        type_ = type(value)
        time.sleep(1)
    ...
    def test_get_without_expiration(self, key, value):
        type_ = type(value)
        if isinstance(value, list):           
            value = json.dumps(value)   
    ...
    def test_get_expired(self, key, value):
        type_ = type(value)
        if isinstance(value, list):            
            value = json.dumps(value)   
        elif isinstance(value, (int, float)):
        ...
        with Popen(["redis-cli", "-p", str(self.PORT), "set", key, value, 'EX', '1'], 
                   stdin=PIPE, stdout=PIPE) as cli:
            output, error = cli.communicate()
            self.assertEqual(output.strip(), b"OK") 
        ...
        self.assertFalse(self.store.get(key))  
...
### ----------------  cash_set|cash_get methods -----------------------
    ...
    def test_cash_get_without_expiration(self, key, value):
        ...
        with Popen(["redis-cli", "-p", str(self.PORT), "set", "cash_" + key, value], 
                   stdin=PIPE, stdout=PIPE) as cli:
            output, error = cli.communicate()
            self.assertEqual(output.strip(), b"OK") 
            
        self.assertEqual(value.encode(), self.store.cache_get(key)) 
